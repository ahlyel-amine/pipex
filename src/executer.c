...
/*   Updated: 2023/02/04 21:33:06 by aahlyel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_execute(t_list **garbg, t_args **args, char **envp)
{
	...
	if (!pid)
	{
		function_faillure(garbg, dup2((*args)->outfile, STDOUT_FILENO), ERRDUP2);
		function_faillure(garbg, execve((*args)->cmds_path[1], (*args)->cmds[1], envp), ERREXEC);
	}
}
...
void	exec_command(t_args *args, t_list **garbg, char **envp)
{
	...
	if (args->infile >= 0)
	{
		...
		if (!pid)
		{
			function_faillure(garbg, dup2(args->fd[1], STDOUT_FILENO), ERRDUP2);
			function_faillure(garbg, close(args->fd[1]), ERRCLOSE);
			function_faillure(garbg, close(args->fd[0]), ERRCLOSE);
			function_faillure(garbg, execve(args->cmds_path[0], args->cmds[0], envp), ERREXEC);
		}
		else
		{
